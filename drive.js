...
function sendfile(chose){
	...
	$.ajax({
		url: 'https://yourdomain.exemple/drivinounet/uploadfile',
		type: 'POST',
		data: formData,
...
function getfile(name){
	...
	xhr.open('POST', "https://yourdomain.exemple/drivinounet/file", true);
...
function getlist(){
	...
	req.open("POST","https://yourdomain.exemple/drivinounet/liste");
...
function sendpassword(){
	var req = new XMLHttpRequest();
	req.responseType = 'json';
	req.open("POST", "https://yourdomain.exemple/drivinounet/login");
	req.setRequestHeader("Content-type","application/json");
	req.onload = function(){
		if(req.readyState == 4 && req.status == 200){
...
function deletefile(path){
	var req = new XMLHttpRequest();
	req.responseType = 'json';
	req.open("DELETE", "https://yourdomain.exemple/drivinounet/file");
	req.setRequestHeader("Content-type","application/json");
	req.onload = function(){
		if(req.readyState == 4 && req.status == 200){
			getlist();
...
function deplacer(){
	var req = new XMLHttpRequest();
	req.responseType = 'json';
	req.open("POST", "https://yourdomain.exemple/drivinounet/mv");
	req.setRequestHeader("Content-type","application/json");
	req.onload = function(){
		if(req.readyState == 4 && req.status == 200){
			getlist();
...
function mkdir(){
	if(document.getElementById("dossier").value != ""){ 
		...
		var req = new XMLHttpRequest();
		req.responseType = 'json';
		req.open("PUT", "https://yourdomain.exemple/drivinounet/mkdir");
		req.setRequestHeader("Content-type","application/json");
		req.onload = function(){
			if(req.readyState == 4 && req.status == 200){
				getlist();
